...
def plot_exponential_smoothing(loss_df, x='time_days', y='loss', hue='run', extra_smoothing=1, xlog=True, ylog=True):
    import plotly.graph_objects as go
    fig = go.Figure()
...
def plot_experiment(experiment_name, x="epoch", verbose=False):
    ...
    fig = plot_exponential_smoothing(losses_all_df, x=x, y='loss')

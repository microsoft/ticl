...
def sample_boolean_data(hyperparameters, seq_len, num_features, device):
    ...
    rank = np.random.randint(1, min(max_rank, num_features_important)) if min(max_rank, num_features_important) > 1 else 1
    n_samples = seq_len
    ...
    inputs_important = inputs[:, torch.randperm(num_features_active)[:num_features_important]]
    while 3 * torch.sum(outputs) < len(inputs):

...
# forward(x: (n_samples, bs, d_model)) -> Tensor of shape (*x.shape[:2],d_model) containing pos. embeddings
...
class LearnedPositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        n_samples, bs, d_model = x.shape
        assert n_samples <= len(self.positional_embeddings), 'n_samples can be at most max_len.'
        pos_emb = self.positional_embeddings[:n_samples]
        return pos_emb.unsqueeze(1).expand(n_samples, bs, d_model) + x  # * math.sqrt(x.shape[-1])
...
class PairedScrambledPositionalEncodings(LearnedPositionalEncoding):
    ...
    def forward(self, x):
        ...
        pos_emb = paired_embs[torch.randperm(len(paired_embs))].view(*self.positional_embeddings.shape)[:n_samples]

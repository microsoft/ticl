...
if '__main__' == __name__:
    ...
    if (not args.head) and os.path.exists('ssm_tabpfn_configs.csv'):
        user_input = input("Old configs detected. Do you want to directly overwrite the original ssm_tabpfn configs? (y/n): ")
        if user_input.lower() in ['y', 'yes']:
            new_configs.to_csv('ssm_tabpfn_configs.csv', index=False, header=True)
            pd.DataFrame(new_configs.index).to_csv('ssm_tabpfn_configs_index.csv', index=False, header=False)
            exit(0)
        
        origin_configs = pd.read_csv('ssm_tabpfn_configs.csv', header=0)
        ...
        additional_configs.to_csv('additional_ssm_tabpfn_configs.csv', index=False, header=True)
        pd.DataFrame(additional_configs.index).to_csv('additional_ssm_tabpfn_configs_index.csv', index=False, header=False)
        
        while 1:
            print('Get additional configs.')
            # Ask the user if they want to overwrite the original configs
            user_input = input("Do you want to overwrite the original ssm_tabpfn configs? (y/n): ")
            if user_input.lower() in ['y', 'yes']:
                new_configs.to_csv('ssm_tabpfn_configs.csv', index=False, header=True)
                pd.DataFrame(new_configs.index).to_csv('ssm_tabpfn_configs_index.csv', index=False, header=False)

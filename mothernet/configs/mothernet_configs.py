...
if '__main__' == __name__:
    ...
    if (not args.head) and os.path.exists('mothernet_configs.csv'):
        user_input = input("Old configs detected. Do you want to directly overwrite the original mothernet configs? (y/n): ")
        if user_input.lower() in ['y', 'yes']:
            new_configs.to_csv('mothernet_configs.csv', index=False, header=True)
            pd.DataFrame(new_configs.index).to_csv('mothernet_configs_index.csv', index=False, header=False)
            exit(0)
        
        origin_configs = pd.read_csv('mothernet_configs.csv', header=0)
        ...
        additional_configs.to_csv('additional_mothernet_configs.csv', index=False, header=True)
        pd.DataFrame(additional_configs.index).to_csv('additional_mothernet_configs_index.csv', index=False, header=False)
        
        while 1:
            print('Get additional configs.')
            # Ask the user if they want to overwrite the original configs
            user_input = input("Do you want to overwrite the original mothernet configs? (y/n): ")
            if user_input.lower() in ['y', 'yes']:
                new_configs.to_csv('mothernet_configs.csv', index=False, header=True)
                pd.DataFrame(new_configs.index).to_csv('mothernet_configs_index.csv', index=False, header=False)
